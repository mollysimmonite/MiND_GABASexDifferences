# =============================================================================
# 04b_young_sex_differences_bilateral.R
# Sex Differences - Young CN, Bilateral Hemisphere Analysis
#
...
#               in young CN adults using bilateral voxel pairs (L/R).
...
# Output:       outputs/04b_young_sex_differences_bilateral_summary.txt
#
...
cat("Young CN sample (wave 1): n =", nrow(young_CN), "\n")
cat("Sex breakdown:\n")
print(table(young_CN$Sex))
cat("\n")

# Voxel pairs: named list of L/R column suffixes
voxel_pairs <- list(
  AUD = c("LAUD", "RAUD"),
  SM  = c("LSM",  "RSM"),
  VV  = c("LVV",  "RVV")
...
run_hemisphere_model <- function(data, metabolite, correction, voxel_name, voxel_pair, label) {
  ...
  # Reshape to long format with Hemisphere as factor
  long_dat <- data %>%
  ...
  for (corr in corrections) {
    for (vox_name in names(voxel_pairs)) {
      ...
      results[[label]] <- run_hemisphere_model(
        young_CN, met, corr, vox_name, voxel_pairs[[vox_name]], label
      )
    }
  }
}
...
summary_lines <- format_bilateral_summary(results, "Bilateral Sex Differences in Young CN Adults (Wave 1)")
cat(paste(summary_lines, collapse = "\n"), "\n")

writeLines(summary_lines, "outputs/04b_young_sex_differences_bilateral_summary.txt")
cat("\nSummary saved to outputs/04b_young_sex_differences_bilateral_summary.txt\n")
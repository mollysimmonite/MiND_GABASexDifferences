# =============================================================================
# 07b_older_MCI_sex_differences_bilateral.R
# Sex Differences - Older MCI, Bilateral Hemisphere Analysis
#
...
#               in older MCI adults (wave 1) using bilateral voxel pairs (L/R).
#               Mixed model: value ~ Sex * Hemisphere + Age + (1|SubNum)
#               Key test: Sex x Hemisphere interaction (lateralisation).
#               Note: smaller sample than CN groups - interpret with caution.
#
# Voxel pairs:  AUD (LAUD/RAUD), SM (LSM/RSM), VV (LVV/RVV)
# Input:        data/MRS_data_analysis.csv
# Output:       outputs/07b_older_MCI_sex_differences_bilateral_summary.txt
#
  ...
  for (corr in corrections) {
    for (vox_name in names(voxel_pairs)) {
      ...
      results[[label]] <- run_hemisphere_model(
        older_MCI, met, corr, vox_name, voxel_pairs[[vox_name]], label
      )
    }
  }
}
...
format_bilateral_summary <- function(results, title) {
  lines <- c(
    ...
    "Key test: Sex x Hemisphere interaction (lateralisation)",
    "Note: smaller MCI sample - interpret with caution",
    "* = p < .05",
    "================================================================",
    ""
  )
...
summary_lines <- format_bilateral_summary(results, "Bilateral Sex Differences in Older MCI Adults (Wave 1)")
cat(paste(summary_lines, collapse = "\n"), "\n")

writeLines(summary_lines, "outputs/07b_older_MCI_sex_differences_bilateral_summary.txt")
cat("\nSummary saved to outputs/07b_older_MCI_sex_differences_bilateral_summary.txt\n")